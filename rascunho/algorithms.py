...
from automoea import MOEATerminationDetection
...
def eaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen,
    ...
    *ngen* generations are done, the algorithm returns a tuple with the final
    ...
    terminator = MOEATerminationDetection(30, 4, 10)
    ...
    while not terminator.done():
        ...
        P = [list(y) for y in [x.fitness.values for x in population[:]]]
        population[:] = toolbox.select(population + offspring, mu)
        Q = [list(y) for y in [x.fitness.values for x in population[:]]]
        ...
        terminator.update(P, Q, gen)
        terminator.status(gen)
        gen += 1
    return population, logbook
